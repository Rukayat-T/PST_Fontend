...
type EditProjectProps = {
  ...
  closeModal: any;
  reset: any;
  selectedProject: IBaseFetchProjectTutor;
};
    ...
    if (selectedProject) {
      setProposalData((prev: any) => {
        return {
          ...
          expectedDeliverable: selectedProject.project_expectedDeliverable,
          resources: selectedProject.project_resources,
          tags: selectedProject.project_tags,
          created_by: profileId as number,
          tutorId: profileId,
          prerequisiteModuleIds: selectedProject.module_ids,
        };
      });
      setCreatedTags(selectedProject.project_tags);
      setCreatedResources(selectedProject.project_resources);
      if (modules.length > 0) {
        const defaultValues = createMultiOptions(
          modules.filter((mod) => selectedProject.module_ids?.includes(mod.id))
        );
      ...
      >
        ...
        <form action="" className="w-full flex flex-col gap-y-5 items-center">
          ...
          <div className="w-full flex items-center gap-3  justify-end">
            ...
            {selectedProject.project_status === "ASSIGNED" ? (
              <button
                type="button"
                disabled={true}
                className="w-fit p-2 bg-lightPurple-20 border-[1.5px] border-darkPurple rounded-lg py-1 mt-5"
              >
                You cannot edit an assigned project
              </button>
            ) : (
              <button
                type="button"
                className="w-fit p-2 bg-lightPurple-20 border-[1.5px] border-darkPurple rounded-lg py-1 mt-5"
                onClick={() => {
                  createProject(selectedProject.project_id, "ACTIVE");
                }}
              >
                {loading ? (
                  <CircularProgress size={20} className="text-darkPurple" />
