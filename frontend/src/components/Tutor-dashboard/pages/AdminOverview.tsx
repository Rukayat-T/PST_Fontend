...
const datap = [
  { name: "Unassigned", value: 0, color: "#8979FF" },
  { name: "Assingned", value: 0, color: "#FF5C53" },
...
function AdminOverview() {
  ...
  const getAdminMetrics = async (id: number) => {
    setIsfetching(true);
    try {
      const res = await DashboardService.getAdminMetric(id);
      if (res.status === 200) {
        setIsfetching(false);
  ...
  const getAdminRecentActivities = async (id: number) => {
    setIsfetching(true);
    try {
  ...
  const getAdminPopularProjects = async () => {
    setIsfetching(true);
    try {
      const res = await DashboardService.getAdminPopularProjects();
      if (res.status === 201) {
        setIsfetching(false);
  ...
  const getAdminAppCountByWeek = async () => {
    setIsfetching(true);
    try {
      const res = await DashboardService.getAdminAppCountByWeek();
      if (res.status === 200) {
        setIsfetching(false);
        setData(
  ...
  const getAdminStatusDistribution = async () => {
    setIsfetching(true);
    try {
      const res = await DashboardService.getAdminStatusDistribution();
      if (res.status === 201) {
        setIsfetching(false);
  ...
  useEffect(() => {
    if (profileId) {
      getAdminMetrics(profileId);
      getAdminRecentActivities(profileId);
      getAdminPopularProjects();
      getAdminStatusDistribution();
      getAdminAppCountByWeek();
    }
  }, [profileId]);
